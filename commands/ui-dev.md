...
```
입력 감지 → /ui-spec → /ui-gen → /ui-verify → PASS/FAIL
                                                  ├─ PASS → 완료
...
감지된 진입점으로 `/ui-spec` 스킬의 절차를 따라 `.ui-spec.json`을 생성한다.

### Stage 2: 코드 생성

`.ui-spec.json`이 생성되면 `/ui-gen` 스킬의 절차를 따라:
...
`/ui-verify` 스킬의 절차를 따라 3단계 검증을 실행한다.
...
2. 실패 원인에 맞는 코드 수정을 적용한다 (targeted edit — /ui-gen 재실행이 아닌 직접 편집)
...
- **3회 실패:** 실패 리포트를 출력하고 개발자 개입을 요청한다:
  "✗ 자동 수정 3회 실패. 아래 항목을 수동으로 수정한 후 /ui-verify로 재검증해주세요."
